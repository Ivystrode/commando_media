...
def delete_idea(request, id):
    ...
    if request.method == "POST":
        if request.user == idea_to_delete.created_by or request.user.profile.role == 'BC' or request.user.profile.role == 'BSM':
...
def edit_idea(request, id):
    print("EDIT ROUTE")
    idea = Idea.objects.get(id=id)
    if request.user == idea.created_by or request.user.profile.role == 'BC' or request.user.profile.role == 'BSM':
        if request.method == "POST":
